...
if [[ $EUID -ne 0 ]]; then
    ...
    if [ $? -ne 0 ]; then
        confirm_prompt "Do you want to add ${username} to the adm group [y]"
        if [ $? -eq 0 ]; then
    ...
    if [ $? -ne 0 ]; then
        confirm_prompt "Do you want to add ${username} to the ssh group [y]"
        if [ $? -eq 0 ]; then
    ...
    if [ $? -ne 0 ]; then
        confirm_prompt "Do you want to add ${username} to the sudo group [y]"
        if [ $? -eq 0 ]; then
...
# TODO: Refactor the code and optimize it! (a lot of repetation)